    ...
    void OnMouseOver()
    {
        if (Input.GetMouseButtonDown(0) && _player == playerColor.WHITE && manager.playerTurn)
        {
            ...
            foreach (Move move in moves)
            {
                ...
                {
                    GameObject instance = Instantiate(Resources.Load("AI_Cube/MoveCube")) as GameObject;
                ...
                {
                    GameObject instance = Instantiate(Resources.Load("AI_Cube/KillCube")) as GameObject;
                    instance.transform.position = new Vector3(-move.secondPosition.Position.x, 0, move.secondPosition.Position.y);
                    instance.GetComponent<Container>().move = move;
                }
            }
            GameObject i = Instantiate(Resources.Load("AI_Cube/CurrentPiece")) as GameObject;
